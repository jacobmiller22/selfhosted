...
import { saveMappings } from "./config.js";
import { parseStatementFile } from "./parsers/index.js";
export function formatBatchSummaryTable(stagedImports) {
    ...
    for (const r of rows) {
        if (r.filename.length > wFile)
            wFile = r.filename.length;
        if (r.accountName.length > wAcc)
            wAcc = r.accountName.length;
        if (r.countStr.length > wCount)
            wCount = r.countStr.length;
        if (r.dateRange.length > wDate)
            wDate = r.dateRange.length;
        if (r.netFlow.length > wNet)
            wNet = r.netFlow.length;
    }
...
export async function promptForCsvProfile(csvHeaders, currentHeaders = {}, filename) {
...
export function formatTransactionPreview(statement, filename) {
...
export function getNextWizardStep(currentStep, choice, context) {
...
export async function resolveFileInteractively(options) {
    ...
    let currentCsvProfile = savedCsvProfile || currentStatement.detectedHeaders || {};
    let selectedAccount;
    let autoMatched = false;
    if (explicitAccountArg) {
        const match = accounts.find(a => a.id === explicitAccountArg || a.name.toLowerCase() === explicitAccountArg.toLowerCase());
        if (match) {
    ...
    if (!selectedAccount && currentStatement.accountNumber) {
        ...
        if (!selectedAccount) {
            const directMatch = accounts.find(a => a.name.includes(currentStatement.accountNumber) || a.id === currentStatement.accountNumber);
            if (directMatch) {
    ...
    let step = (isCsv && !savedCsvProfile) ? "CSV_MAPPING" : (selectedAccount ? "REVIEW" : "ACCOUNT_SELECTION");
    let shouldSaveRule = true;
    let genericMatchChoice = "exact";
    while (true) {
        if (step === "CSV_MAPPING") {
        ...
        if (step === "ACCOUNT_SELECTION") {
            ...
            const saveRulePromptAnswer = await inquirer.prompt([
                {
                    type: "confirm",
...
export async function resolveAccountForFile(filePath, statement, accounts, mappings, mappingsPath, explicitAccountArg) {
...
export function getFilenameBaseAndIsGeneric(filename) {
    ...
    const nonGenericTokens = tokens.filter(token => !GENERIC_KEYWORDS.includes(token) && !/^\d+$/.test(token));
