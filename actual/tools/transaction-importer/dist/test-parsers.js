...
import { formatCsvMappingRundown, formatTransactionPreview, formatFinalReviewBox, getRuleToSaveDescription, getNextWizardStep, formatBatchSummaryTable } from './matcher.js';
...
if (parsedOfx.accountStatements.length === 1 &&
...
if (multiQifParsed.accountStatements.length === 2 &&
...
if (multiOfxParsed.accountStatements.length === 2 &&
...
if (previewOutput.includes('export_august.csv') &&
...
if (capOneStmt.transactions.length === 2 &&
...
if (singleAmtStmt.transactions.length === 2 &&
...
if (rundownOutput.includes('test_statement.csv') &&
...
if (customStmt.transactions.length === 2 &&
...
if (reviewBoxCsv.includes('FINAL REVIEW FOR "file.qif"') &&
...
if (desc1 === "Match account number (12345)" &&
...
if (scalarTypeStmt.transactions.length === 4 &&
...
if (batchSummaryOutput.includes('File Name') &&
...
if (extStmt.transactions.length === 4 &&
...
if (balanceTableOutput.includes('360 Checking') &&
