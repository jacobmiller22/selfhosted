...
import {
  ...
  getFilenameBaseAndIsGeneric,
  resolveFileInteractively,
  type StagedImport
} from "./matcher.js";
...
program
  ...
  .action(async (targetPathArg: string, options: { account?: string; dryRun: boolean; archive: boolean; maxTransferDays: string; transfers: boolean }) => {
    ...
    try {
      ...
      const stagedImports: StagedImport[] = [];
      ...
      console.log(`\n================ Phase 2: Execution & Archiving ================`);
      const successfulFilePaths = new Set<string>();
      const importSummary: Array<{ filename: string; account: string; count: number; status: string }> = [];
